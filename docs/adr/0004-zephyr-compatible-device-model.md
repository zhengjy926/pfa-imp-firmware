...
- **抽象五类总线 + LED 设备类**：spi / i2c / uart / gpio / eeprom（API 形状对齐 Zephyr `drivers/*.h`），另增 `led` 设备类（形状对齐 Zephyr `drivers/led.h` 的 `led_on` / `led_off`，叠在 gpio 上）。HAL 实现在 bsp 层注册为设备；AD5941（spi）、TCA6424A（gpio）、AT24C256C（eeprom）、状态指示灯（led）挂对应类。本板先做 1 颗状态指示灯。LED 不是领域术语，不进 `CONTEXT.md`。
