...
下文先单独列出调研当时的项目输入，再按 6 问写文献结论。二者不得混读。现行领域定义以仓库根目录 `CONTEXT.md` 为准，不要用本节条目覆盖术语表。

---

## 本项目已确认事实（非文献；2026-08-25 调研输入，已部分过期）

以下是调研任务当时的项目输入，**不是现行需求**。导管类型、放电构型、放电准备与贴靠继电器以 `CONTEXT.md`、ADR 0005/0006 及 `docs/protocol/loop-impedance-board.md` 为准。过期要点：现行为三种导管（PVI一代无顶电极；PVI二代 6×3+顶；局灶 6×2+顶），不是单一 25 位点；放电隔离是上位机放电准备命令，无硬件 Blank。

下列条目保留作文献对照，不作为行业证据：

- 导管（调研当时）：6 杆 × 4 杆电极 + 1 顶电极 = 25 位点。
...
8. FARASTAR 类 **blanking 同步信号** 是否存在于本发生器（项目需求写测量仅放电前；触发源仍待硬件补充）。**已决（ADR 0005）**：不存在、也不需要硬件 Blank；放电前隔离只响应放电准备命令，两板无数字互连。
